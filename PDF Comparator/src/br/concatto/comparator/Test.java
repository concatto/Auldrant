...
public class Test extends PDFStreamEngine
{
    ...
    private static final String thisClassName = Test.class.getSimpleName().toLowerCase();
     ...
     */
    public Test( Properties props ) throws IOException
    {
        super( props );
        this.outputEncoding = null;
        normalize = new TextNormalize(this.outputEncoding);
     ...
     * @throws IOException If there is an error reading the properties.
     */
    public Test( String encoding ) throws IOException
    {
        super( ResourceLoader.loadProperties(
                "org/apache/pdfbox/resources/PDFTextStripper.properties", true ));
    ...
    /**
     * @deprecated
     * @see Test#writeText( PDDocument, Writer )
    ...
    protected void processPage( PDPage page, COSStream content ) throws IOException
    {
                ...
                (endBookmarkPageNumber == -1 || currentPageNo <= endBookmarkPageNumber ))
        {
            ...
            new ExplosionFilter().decode(content.getFilteredStream(), out, content, 0);
//            System.out.println(out.toString());
    ...
    static class ExplosionFilter extends FlateFilter {
		public void decode(InputStream compressedData, OutputStream result,
			...
			COSBase baseObj = options.getDictionaryObject(COSName.DECODE_PARMS,
					COSName.DP);
			...
			if (baseObj instanceof COSDictionary) {
				...
				throw new IOException(
						"Error: Expected COSArray or COSDictionary and not "
			...
			try {
				...
				if (predictor > 1) {
					...
					int bitsPerPixel = dict.getInt(COSName.BITS_PER_COMPONENT,
							8);
					...
					ByteArrayInputStream bais = new ByteArrayInputStream(
							baos.toByteArray());
//					Predictor.decodePredictor(predictor, colors, bitsPerPixel,
//							columns, bais, result);
					result.flush();
					baos.reset();
					bais.reset();
...
//				LOG.error("FlateFilter: stop reading corrupt stream due to a DataFormatException");
		...
		// Use Inflater instead of InflateInputStream to avoid an EOFException
		// due to a probably
